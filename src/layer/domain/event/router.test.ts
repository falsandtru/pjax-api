...
describe('Unit: layer/domain/event/router', () => {
  describe('RouterEventRequest', () => {
    it('click', () => {
      ...
      assert(req.url.reference === standardize(''));
  ...
  describe('RouterEventLocation', () => {
    it('instance', () => {
      ...
      assert(loc.orig.reference === standardize(window.location.href));
      assert(loc.dest.reference === standardize(window.location.href + '#'));
    });
