...
###pjaxの導入ポイント
pjaxにより恩恵が得られるのは基本的にサーバーサイドでの高速化の代替、非常に重いページ初期化処理の省略、ページ状態の維持を目的として使用する場面であり、これらいずれにも該当しない場合（サーバーでブラウザがHTML、CSS、JavaScriptをキャッシュするよう設定しており、ページの初期化処理（JavaScriptの実行）が重くなく、ページ状態を維持する必要がない）は余分な処理をさせる分デメリットのほうが大きくなるため使用しないべきです。また、pjaxをサイトの一部のページだけで使用するための機能としてscope機能が用意されています。ただし、FirefoxはすべてのデータをキャッシュしてもJavaScriptの実行速度の遅さからページ移動が遅くなりやすいためpjaxを使用したほうが高速です。
