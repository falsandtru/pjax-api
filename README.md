...
|部分的更新キャンセル※6|×|○|
|比較用デモ※7|<a href="http://sa-kusaku.sakura.ne.jp/output/pjax/demo/defunkt/" target="_blank">defunkt</a>|<a href="http://sa-kusaku.sakura.ne.jp/output/pjax/demo/falsandtru/" target="_blank">falsandtru</a>|
...
**<a href="http://sa-kusaku.sakura.ne.jp/output/pjax/demo/server/" target="_blank">demo</a>** ※これは移動先のページを差分データに置き換えた擬似的なデモです。  
...
+ パラメータの仕様を変更
  <br>すべてのコールバック関数のコンテキストを<code>null</code>に変更。
...
+ パラメータを追加
  <br><code>pushState</code>の第一引数として渡す値を設定する。関数が設定された場合は戻り値が渡される。
...
+ scope機能を修正
  ...
  <br>inherit機能を追加。
+ パラメータの仕様を変更
  <br>条件の設定方法を組み合わせ先のみ<code>{'/a/': ['/b/']}</code>）から自身を含めた組み合わせ（範囲）<code>{'/a/': ['/a/', '/b/']}</code>）に変更。
  <br>複数条件の設定方法を<code>|</code>区切りの文字列（<code>'a|b'</code>）から配列による列挙（<code>['a','b']</code>）に変更。
+ パラメータに設定された関数に第二引数を追加、移動先のURLが渡される
+ スクロール処理を修正
...
+ スクロール機能を修正
  <br><code>scrollTop</code>および<code>scrollLeft</code>の設定値ないし戻り値が無効であった場合に使用される設定値を<code>null</code>から<code>0</code>に変更。
　
...
+ script要素のtype属性値の確認処理を追加
  <br>type属性に<code>text/javascript</code>以外の値が設定されている埋め込み型JavaScriptを実行しない。
