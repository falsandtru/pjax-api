...
###pjaxの導入にあたって
pjaxの恩恵が得られるのは基本的にサーバーサイドでの高速化の代替、重いページ初期化処理の省略、ページ状態の維持を目的として使用する場面であり、これらいずれにも該当しない場合（サーバーでブラウザがHTML、CSS、JavaScriptをキャッシュするよう設定しており、ページの初期化処理（JavaScriptの実行）が重くなく、ページ状態を維持する必要がない）は余分な処理をさせるデメリットが生じるだけであるため使用をよく検討するべきです。pjaxをサイトの一部のページだけで使用する場合はscope機能の使用を推奨します。

###pjaxのコンテンツ別使用適否
####テキストコンテンツのサイト
テキストコンテンツを主体とするサイトではあまりpjaxを使用するべきではありません。テキストを読み進める場合、ページ移動時のホワイトアウトがページの余白として機能するためこれを失わせると読者がページから意識をはずして休息をとる1秒前後の時間やページを読み進めるリズムを奪うことになります。
