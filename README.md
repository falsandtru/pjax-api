...
フルフラッシュサイトのHTML5化、ウェブサービスのネイティブアプリ様の提供、ウェブアプリ開発におけるajaxの機能拡張にも利用できます。
...
pjaxによりページ移動を行うリンク（アンカータグ）をjQueryセレクタで選択します。初期値は`target`属性がない`anchor`要素です。
カレントディレクトリを基準とした相対パスは正常に動作しない可能性があるため使用しないでください。
...
###pjaxの導入のポイント
pjaxにより恩恵が得られるのは基本的にサーバーサイドでの高速化の代替、重いページ初期化処理の省略、ページ状態の維持を目的として使用する場面であり、これらいずれにも該当しない場合（サーバーでブラウザがHTML、CSS、JavaScriptをキャッシュするよう設定しており、ページの初期化処理（JavaScriptの実行）が重くなく、ページ状態を維持する必要がない）は余分な処理をさせる分デメリットのほうが大きくなるため使用しないべきです。また、pjaxをサイトの一部のページだけで使用するための機能としてscope機能が用意されています。ただし、FirefoxはすべてのデータをキャッシュしてもJavaScriptの実行速度の遅さからページ移動が遅くなりやすいためpjaxの使用が検討できます。
...
このように、pjaxはajaxの利便性を拡張するものとしてウェブアプリケーションの分野で非常に有用であり、今後どのように利用されるか気になるところです。
