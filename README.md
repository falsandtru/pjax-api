...
ブラウザがキャッシュを保持していると予測できる場合にpjaxをキャンセルして通常のページ移動を行います。プリロードによる高速化のみにpjaxを使用する場合に有効です。
キャッシュを保持しているとみなす、ajaxによるページ取得に要した時間をミリ秒で設定します。設定値には`50`ミリ秒程度を推奨します。初期値は`0`で無効です。

#####*cache.expires: node*
キャッシュの有効期限にかかる設定項目を持ちます。

#####*cache.expires.min: Millisecond as number*
キャッシュの有効期限の最小値をミリ秒で設定します。初期値は`1800000`(30分)です。

#####*cache.expires.max: Millisecond as number*
キャッシュの有効期限の最大値をミリ秒で設定します。初期値は`null`(無効)です。
