...
<small>※動作テストのためpjaxが正常に動作していないことがあります。恐縮ですがその際は時間をおいて再度ご覧ください。</small>
...
###更新情報
大幅な機能追加を矢継ぎ早に行ったため環境によってはエラーが発生する可能性があります。

####change log

#####1.20.7

+ <code>fix</code>パラメータを追加
  <br><small>pjaxの諸問題の修正を行うかを設定する。</small>
+ パラメータの動作を修正
+ パラメータの動作を修正
...
+ パラメータを追加
  <br><small>pjaxではブラウザバックでスクロール位置が復元されないことがある問題に対応。</small>
+ データベース処理のバグを修正
...
+ パラメータの仕様を変更
  <br><small>すべてのコールバック関数のコンテキストを<code>null</code>に変更。</small>
+ パラメータに設定された関数に第二引数を追加、移動先のURLが渡される
+ コードを最適化

#####1.20.1

+ パラメータを追加
  <br><small><code>pushState</code>の第一引数として渡す値を設定する。関数が設定された場合は戻り値が渡される。</small>

#####1.20.0

+ パラメータを追加
  <br><small>ajax通信が強制終了された場合に間違った履歴が記録される問題に対応。</small>
...
+ scope機能を修正
  <br><small>正規表現を使用できるよう修正。</small>
  <br><small>rewrite機能を追加。</small>
  <br><small>inherit機能を追加。</small>
+ パラメータの仕様を変更
  <br><small>条件の設定方法を組み合わせ先のみ<code>{'/a/': ['/b/']}</code>）から自身を含めた組み合わせ（範囲）<code>{'/a/': ['/a/', '/b/']}</code>）に変更。</small>
  <br><small>複数条件の設定方法を<code>|</code>区切りの文字列（<code>'a|b'</code>）から配列による列挙（<code>['a','b']</code>）に変更。</small>
+ パラメータに設定された関数に第二引数を追加、移動先のURLが渡される
+ スクロール処理を修正
...
+ scope機能を修正
  <br><small>ページ単位で範囲を設定できるよう修正。</small>
  <br><small>サブディレクトリで設定を上書きできるよう修正。</small>
+ scope機能のバグを修正

#####1.19.0

+ パラメータを追加
  <br><small>pjaxにより移動するURLの範囲を設定、制限する。</small>
...
+ キャッシュ機能のバグを修正
  <br><small>submitによる移動先ページをキャッシュできないバグを修正。</small>
+ パラメータを追加。
...
+ ajax処理を修正
  <br><small>pjaxによる前回のajax通信が終了していない場合にこの通信を強制終了させてから次の通信を開始するよう修正。</small>
...
+ version: 1.16.4 - 1.17.8</small>
  <br><small>pjaxによるページ移動処理中にさらにページ移動をしようとした場合に通常のページ移動が行われることがあるバグを修正。</small>
...
+ スクロール機能を修正
  <br><small><code>scrollTop</code>および<code>scrollLeft</code>の設定値ないし戻り値が無効であった場合に使用される設定値を<code>null</code>から<code>0</code>に変更。</small>
　
...
+ version: 1.15.0 - 1.16.2</small>
  <br><small>JavaScriptの同期読み込みを有効にしていない場合に更新反映の確認処理が無限ループするバグを修正。</small>
+ スクロール機能のバグを修正
+ コードを高速化
...
+ 更新反映の確認機能の汎用化
+ スクロール機能の実装方法を修正
  <br><small>モバイル端末のブラウザでスクロールされない場合があるバグを修正。</small>
+ コードを高速化
...
+ script要素のtype属性値の確認処理を追加
  <br><small>type属性に<code>text/javascript</code>以外の値が設定されている埋め込み型JavaScriptを実行しない。</small>
...
+ URLと表示されているページが一致しない場合があるバグを修正
  <br><small>ブラウザの操作（戻る進むボタンの連打等）によりページ移動が非常に高速に行われた場合に発生。</small>
