...
b,[c,a.parameter,l]));var n=jQuery(h),s=0<n.filter("title").length;x=x?x:s?n.filter("title").text():B(h,"<title>([^<]*)</title>");var v=a.area.split(","),w=null===a.scrollLeft?jQuery(p).scrollLeft():parseInt(a.scrollLeft),A=null===a.scrollTop?jQuery(p).scrollTop():parseInt(a.scrollTop);if(!jQuery(a.area).length||!n.find(a.area).add(n.filter(a.area)).length)throw Error();if(!1!==j(a.callbacks.update.url.before,b,[c,a.parameter,h,m,f])){k=k.replace(RegExp("[?&]"+a.server.query+"=[^&#]*"),"");e&&p.history.pushState("pjax",
p.opera||"userAgent"in p&&-1!==userAgent.indexOf("opera")?x:C.title,k);switch(!0){case /Mobile(\/\w+)? Safari/i.test(p.navigator.userAgent):a.disable=!0,p.history.back(),p.history.forward(),a.disable=!1}j(a.callbacks.update.url.after,b,[c,a.parameter,h,m,f])}!1!==j(a.callbacks.update.title.before,b,[c,a.parameter,h,m,f])&&(C.title=x,j(a.callbacks.update.title.after,b,[c,a.parameter,h,m,f]));if(!1!==j(a.callbacks.update.content.before,b,[c,a.parameter,h,m,f])){for(var r=0,y;y=v[r];r++)jQuery(y).html(n.find(y).add(n.filter(y)).html()),
