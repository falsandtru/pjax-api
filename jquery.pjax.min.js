...
h.id=1;d.ids.push(h.id);d.settings[h.id]=h;n&&n.test("++",1,0,"register");d.check()&&(d.share(),d.registrate.call(f,a,c,e,s,d,h));n&&n.end();return f}var k;k={name:"pjax",alias:"",ids:[0],settings:[0],count:0,parser:null,setAlias:function(a){k.alias="string"===typeof a?a:k.alias;k.name===k.alias||l[k.alias]||(l[k.name][k.alias]=l.fn[k.name],l.fn[k.alias]=l[k.alias]=l.fn[k.name])},setProperties:function(a,c){var e=this;if(e instanceof l||e===l[k.name])e=e instanceof l&&c!==x?e.add(c):e,e[k.name]=l[k.name],
e.on=function(){k.settings[1].disable=!1},e.off=function(){k.settings[1].disable=!0},e.click=function(a,d){d=d||{};return(d.href=a)?l("<a/>",d).one("click",1,k.click).click():!1},e.submit=function(a,d,b){var c=B.createDocumentFragment(),f;switch(!0){case "object"===typeof a:d=l(a);break;case !!b:d=d||{};d.action=a;a=b instanceof Array&&Array||b instanceof Object&&Object||x;for(var h in b){f=b[h];switch(a){case Object:f=l("<textarea/>",{name:h}).val(f);break;case Array:f.attr=f.attr||{};f.attr.name=
f.name;f=l(f.tag.indexOf("<")?"<"+f.tag+"/>":f.tag,f.attr||{}).val(f.value);break;default:continue}c.appendChild(f[0])}d=l("<form/>",d).append(c);break;default:return!1}d.one("submit",1,k.submit).submit();return!0},e.setCache=function(a,d,b,c){var f=k.settings[1],h,e,u;h=f.history;a=a||k.canonicalizeURL(r.location.href);f.hashquery||(a=a.replace(/#.*/,""));switch(arguments.length){case 1:for(e=0;u=h.order[e];e++)a===u&&(h.order.splice(e,1),h.size-=h.data[u].size,h.data[u]=null,delete h.data[u]);break;
case 0:return arguments.callee.call(this,a,k.trim(B.documentElement.outerHTML));default:h.order.unshift(a);for(e=1;u=h.order[e];e++)a===u&&h.order.splice(e,1);h.size>h.config.size&&l[k.name].cleanCache();l[k.name].getCache(a);e=l("<span/>").html(k.find((d||"")+((c||{}).responseText||"")+"<title></title>",/<title[^>]*?>([^<]*?)<\/title>/i).shift()).text();u=parseInt(1.8*((d||"").length+((c||{}).responseText||"").length)||1048576,10);h.size=h.size||0;h.size+=u;h.data[a]=l.extend(!0,h.data[a]||{},{XMLHttpRequest:c,
textStatus:b,data:d,size:u,timestamp:(new Date).getTime()});f.database&&f.fix.history&&k.dbTitle(a,e)}return h.data[a]},e.getCache=function(a){var d=k.settings[1],b;b=d.history;a=a||k.canonicalizeURL(r.location.href);d.hashquery||(a=a.replace(/#.*/,""));b.data[a]&&d.timestamp>b.data[a].timestamp+b.config.expire&&l[k.name].setCache(a);return b.data[a]},e.clearCache=function(){for(var a=k.settings[1].history,d=a.order.length,b;b=a.order[--d];)a.order.splice(d,1),a.size-=a.data[b].size,delete a.data[b];
...
a(c).scrollTop())},h.scroll.delay);h.scroll.queue.push(e)}else d.dbScroll(a(c).scrollLeft(),a(c).scrollTop());h.scroll.suspend&&!f&&(a(this).unbind(h.nss.scroll),setTimeout(function(){h.database&&h.fix.scroll&&a(c).bind(h.nss.scroll,h.id,k).trigger(h.nss.scroll,[!0])},h.scroll.suspend))});(function(){function b(c){try{var d=c&&c.parseFromString&&c.parseFromString("<body><noscript>DOMParser</noscript></body>","text/html");return"DOMParser"===a(d).find("noscript").text()}catch(f){}}var f=c.DOMParser;
d.parser=f&&f.prototype&&new f;b(d.parser)||(d.parser=function(){},d.parser.parseFromString=function(a,b){if(/(?:^|.+?\s+?|\s*?)text\/html(?:[\s;]|$)/i.test(b)){if(e.implementation&&e.implementation.createHTMLDocument){var c=e.implementation.createHTMLDocument("");"object"===typeof c.activeElement&&(c.open(),c.write(a),c.close());return c}}else return(new f).parseFromString.apply(this,arguments)},b(d.parser)||(d.parser=!1))})()},drive:function(a,c,e,k,d,b,n,f,h,l){function u(a,b,c,d,e,g,k,l){var m=
g.validator?g.validator.clone({name:"jquery.pjax.js - update()"}):!1;m&&m.start();m&&(m.scope=function(a){return eval(a)});m&&m.test("1",1,0,"update()");m&&m.test("++",1,0,"start");a:{var s=g.speedcheck;s&&g.log.speed.name.push("update");s&&g.log.speed.time.push(g.speed.now()-g.log.speed.fire);var p=g.callbacks.update;if(!1!==e.fire(p.before,null,[k,g.parameter,w,t,q,l],g.callbacks.async)){var n,r,u;try{m&&m.test("++",1,0,"try");m&&m.test("++",1,l?0:[g.contentType,q.getResponseHeader("Content-Type")],
"content-type");if(!l&&-1===(q.getResponseHeader("Content-Type")||"").toLowerCase().search(g.contentType))throw Error("throw: content-type mismatch");m&&m.test("++",l?"'usable'":"'unusable'",0,"cache");l&&!1!==e.fire(p.cache.load.before,null,[k,g.parameter,l],g.callbacks.async)&&(q=l.XMLHttpRequest||q,w=q.responseText,t=l.textStatus||t,r=l.css,u=l.script,e.fire(p.cache.load.after,null,[k,g.parameter,l],g.callbacks.async));m&&m.test("++",1,0,"initialize");var z,A,y,x,v,B,D;B=g.area.split(/\s*,\s*/);
A=(q.responseText||"").replace(/<noscript[^>]*>(?:.|[\n\r])*?<\/noscript>/gim,function(a){return a.replace(/<script(?:.|[\n\r])*?<\/script>/gim,"")});if(l&&l.data){x=l.data;y=a(e.parser&&x&&e.parser.parseFromString(x,"text/html")||x);A=A.replace(/<title[^>]*?>([^<]*?)<\/title>/i,function(a){return e.find(x,/(<title[^>]*?>[^<]*?<\/title>)/i).shift()||a});z=a(e.parser&&A&&e.parser.parseFromString(A,"text/html")||A);for(var I=0,C,E;C=B[I++];)z.find(C).add(v?"":z.filter(C)).html(y.find(C).add(v?"":y.filter(C)).contents())}else z=
a(e.parser&&A&&e.parser.parseFromString(A,"text/html")||A);switch(!0){case !!z.find("html")[0]:v=1;z.find("noscript").each(function(){this.children.length&&a(this).text(this.innerHTML)});break;case !!z.filter("title")[0]:v=0;break;default:v=!1}switch(v){case 1:n=z.find("title").text();break;case 0:n=z.filter("title").text();break;case !1:n=a("<span/>").html(e.find(A,/<title[^>]*?>([^<]*?)<\/title>/i).shift()).text()}if(!a(g.area).length||!z.find(g.area).add(v?"":z.filter(g.area)).length)throw Error("throw: area length mismatch");
a(b).trigger(g.gns+".unload");m&&m.test("++",1,f,"url");!1!==e.fire(p.url.before,null,[k,g.parameter,w,t,q],g.callbacks.async)&&(h&&f!==g.location.href&&b.history.pushState(e.fire(g.state,null,[k,f]),b.opera||-1!==b.navigator.userAgent.toLowerCase().indexOf("opera")?n:c.title,f),g.location.href=f,h&&g.fix.location&&(a[e.name].off(),b.history.back(),b.history.forward(),a[e.name].on()),e.fire(p.url.after,null,[k,g.parameter,w,t,q],g.callbacks.async));m&&m.test("++",1,n,"title");!1!==e.fire(p.title.before,
