...
f.importNode?f.importNode(x,!0):x.cloneNode(!0);switch(x.tagName.toLowerCase()){case "base":C="*";break;case "link":C='[rel="'+x.getAttribute("rel")+'"]';switch((x.getAttribute("rel")||"").toLowerCase()){case "alternate":C+="string"===typeof x.getAttribute("type")?'[type="'+x.getAttribute("type")+'"]':":not([type])"}break;case "meta":if(x.getAttribute("charset"))C="[charset]";else if(x.getAttribute("http-equiv"))C='[http-equiv="'+x.getAttribute("http-equiv")+'"]';else if(x.getAttribute("name"))C=
'[name="'+x.getAttribute("name")+'"]';else if(x.getAttribute("property"))C='[property="'+x.getAttribute("property")+'"]';else continue;break;default:C=null}J=w.filter(x.tagName).filter(C);a("head").children().filter(x.tagName).filter(C).not(c.load.reload).not('link[rel~="stylesheet"], style, script').filter(function(){var a=this,b;return!!J.filter(function(){b=this;return a.outerHTML===b.outerHTML?(J=J.not(b),N=N.not(a),!1):!0})[0]}).remove();a("head").prepend(J.map(function(){return a(this.outerHTML)[0]}))}N.not(c.load.reload).remove();
