 ...
 */
(function(){function t(p){function t(g,b,e,n,f){function s(m){j&&j.test("++",1,0,"update()");j&&(j.scope=function(a){return eval(a)});a:if(a.speedcheck&&a.log.speed.name.push("update_start"),a.speedcheck&&a.log.speed.time.push(a.speed.now()-a.log.speed.fire),!1!==k(a.callbacks.update.before,g,[b,a.parameter,l,h,d])){var f,q,p;try{if(!m&&!RegExp(a.contentType.replace(/\s*[,;]\s*(.|$)/g,function(a,g){return g?"|"+g:""}),"i").test(d.getResponseHeader("Content-Type")))throw Error();m&&!1!==k(a.callbacks.update.cache.load.before,
g,[b,a.parameter,m])&&(d=m.XMLHttpRequest,l=d.responseText,h=m.dataType,f=m.title,q=m.css,p=m.script,k(a.callbacks.update.cache.load.after,g,[b,a.parameter,m]));var v=jQuery(l),t=0<v.filter("title").length,y=a.area.replace(/(\((.*?(\(.*?\).*?)?)*?\)|\S)(,|$)/g,function(a,g,b,d,e){return g+(e?"|":"")}).split(/\s*\|(?!=)\s*/),x=null===a.scrollLeft?jQuery(c).scrollLeft():parseInt(a.scrollLeft),H=null===a.scrollTop?jQuery(c).scrollTop():parseInt(a.scrollTop);f=f?f:t?v.filter("title").text():jQuery("<span/>").html(D(l,
"<title>([^<]*)</title>").join()).text();if(!jQuery(a.area).length||!v.find(a.area).add(v.filter(a.area)).length)throw Error();if(!1!==k(a.callbacks.update.url.before,g,[b,a.parameter,l,h,d])){e=e.replace(RegExp("[?&]"+a.server.query+"=[^&#]*"),"");n&&c.history.pushState(a.gns,c.opera||-1!==c.navigator.userAgent.toLowerCase().indexOf("opera")?f:E.title,e);switch(!0){case /Mobile(\/\w+)? Safari/i.test(c.navigator.userAgent):c.history.back(),c.history.forward()}k(a.callbacks.update.url.after,g,[b,a.parameter,
l,h,d])}!1!==k(a.callbacks.update.title.before,g,[b,a.parameter,l,h,d])&&(E.title=f,k(a.callbacks.update.title.after,g,[b,a.parameter,l,h,d]));if(!1!==k(a.callbacks.update.content.before,g,[b,a.parameter,l,h,d])){for(var A=0,z;z=y[A];A++)jQuery(z).html(v.find(z).add(v.filter(z)).children()),a.load.script&&a.load.sync&&jQuery(z).append(jQuery("<div/>",{"class":a.nss.class4html+"-loaded",style:"display: block !important; visibility: hidden !important; width: auto !important; height: 0 !important; margin: 0 !important; padding: 0 !important; border: none !important; position: absolute !important; top: -9999px !important; left: -9999px !important; font-size: 12px !important; text-indent: 0 !important;"}).text("pjax"));
k(a.callbacks.update.content.after,g,[b,a.parameter,l,h,d])}a.load.css&&setTimeout(function(){C()},a.load.async||0);var C=function(){var c=r[a.id]&&r[a.id].validate?r[a.id].validate.clone({name:"jquery.pjax.js :css()"}):c;c&&c.start();c&&(c.scope=function(a){return eval(a)});if(!1!==k(a.callbacks.update.css.before,g,[b,a.parameter,l,h,d])){q=q?q:t?v.find('link[rel="stylesheet"], style').add(v.filter('link[rel="stylesheet"], style')):D(l,'(<link[^>]*?rel="stylesheet"[^>]*?>|<style[^>]*?>(.|[\n\r])*?</style>)');
w(a.history,e)&&(w(a.history,e).css=q);jQuery('link[rel="stylesheet"], style').filter(function(){return jQuery.data(this,a.nss.data,!0)});for(var f=0,m,j=jQuery('link[rel="stylesheet"]'),n=jQuery("style"),p;m=q[f];f++){p=!1;m=t?m:jQuery(m)[0];if("LINK"===m.tagName.toUpperCase()&&j.filter(function(){if(p||this.href!==m.href)return!1;p=!0;jQuery.data(this,a.nss.data)&&jQuery.data(this,a.nss.data,!1);return!0}).length)continue;if("STYLE"===m.tagName.toUpperCase()&&n.filter(function(){if(p||!jQuery.data(this,
a.nss.data)||this.innerHTML!==m.innerHTML)return!1;p=!0;jQuery.data(this,a.nss.data,!1);return!0}).length)continue;jQuery.data(jQuery("head").append(m).children(":last-child")[0],a.nss.data,!1);m=null}jQuery('link[rel="stylesheet"], style').filter(function(){return jQuery.data(this,a.nss.data)}).remove();!1!==k(a.callbacks.update.css.after,g,[b,a.parameter,l,h,d])&&(a.speedcheck&&a.log.speed.name.push("css"),a.speedcheck&&a.log.speed.time.push(a.speed.now()-a.log.speed.fire))}c&&c.end()};a.load.script&&
setTimeout(function(){F("async")},a.load.async||0);var F=function(c){var m=r[a.id]&&r[a.id].validate?r[a.id].validate.clone({name:"jquery.pjax.js :script()"}):m;m&&m.start();m&&(m.scope=function(a){return eval(a)});if(!1!==k(a.callbacks.update.script.before,g,[b,a.parameter,l,h,d])){var f=[];p=p?p:t?v.find("script").add(v.filter("script")):D(l,"(?:[^'\"]|^s*)(<script[^>]*?>(.|[\n\r])*?\x3c/script>)(?:[^'\"]|s*$)");w(a.history,e)&&(w(a.history,e).script=p);for(var j=0,n;n=p[j];j++)if(n=t?n:jQuery(n)[0],
"sync"!==c||n.defer)if(!("async"===c&&n.defer)&&(n.childNodes.length||!(n.src in a.log.script)))n.src&&(a.log.script[n.src]=!0),jQuery.when?(n=n.src?jQuery.ajax(jQuery.extend(!0,{},a.ajax,{url:n.src,dataType:"script",async:!1,global:!1})):jQuery.Deferred().resolve(n),f.push(n)):jQuery("head").append(n),n=null;jQuery.when&&f.length&&jQuery.when.apply(null,f).done(function(){for(var a=0,g;g=arguments[a];a++)0<Number(g.nodeType)&&eval((g.text||g.textContent||g.innerHTML||"").replace(/^\s*<!(?:\[CDATA\[|\-\-)/,
"/*$0*/"))});k(a.callbacks.update.script.after,g,[b,a.parameter,l,h,d])}m&&m.end()};a.load.script&&a.load.sync?setTimeout(function(){jQuery(a.area).length===jQuery(a.area).children("."+a.nss.class4html+"-loaded").filter(function(){return this.clientWidth}).length?(jQuery(a.area).children("."+a.nss.class4html+"-loaded").remove(),setTimeout(function(){F("sync")},0),a.speedcheck&&a.log.speed.name.push("script"),a.speedcheck&&a.log.speed.time.push(a.speed.now()-a.log.speed.fire),a.speedcheck&&console.log(a.log.speed.time),
a.speedcheck&&console.log(a.log.speed.name)):setTimeout(function(){arguments.callee()},a.interval)},0):a.load.script&&setTimeout(function(){F("sync")},a.load.async||0);switch(b.type.toLowerCase()){case "click":c.scrollTo(x,H);break;case "submit":c.scrollTo(x,H);break;case "popstate":c.history.state instanceof Object&&isFinite(c.history.state.scrollY)&&(c.scrollTo(x,c.history.state.scrollY),c.history.state.scrollY=u,c.history.replaceState(c.history.state,f,e))}if((a.cache.click||a.cache.submit||a.cache.popstate)&&
"POST"!==a.ajax.type&&!1!==k(a.callbacks.update.cache.save.before,g,[b,a.parameter,m]))w(a.history,e,f,B,l,h,d),k(a.callbacks.update.cache.save.after,g,[b,a.parameter,m]);if(!1===k(a.callbacks.update.success,g,[b,a.parameter,l,h,d]))break a;if(!1===k(a.callbacks.update.complete,g,[b,a.parameter,l,h,d]))break a;if(!1===k(a.callback,g,[b,a.parameter,l,h,d]))break a;j&&j.test("++",1,0,"update: success")}catch(I){m&&w(a.history,e,f,null);if(!1===k(a.callbacks.update.error,g,[b,a.parameter,l,h,d]))break a;
if(!1===k(a.callbacks.update.complete,g,[b,a.parameter,l,h,d]))break a;j&&j.test("++",1,[e,c.location.href],"update: error");if(a.fallback)return"function"===typeof a.fallback?a.fallback(b):G(b)}!1!==k(a.callbacks.update.after,g,[b,a.parameter,l,h,d])&&(a.on(),a.speedcheck&&a.log.speed.name.push("end"),a.speedcheck&&a.log.speed.time.push(a.speed.now()-a.log.speed.fire),j&&j.test("++","url === win.location.href",[e,c.location.href],"update: is updated"),j&&j.end())}}var j=r[a.id]&&r[a.id].validate?
r[a.id].validate.clone({name:"jquery.pjax.js :drive()"}):j;j&&j.start();j&&(j.scope=function(a){return eval(a)});j&&j.test(1,1,0,"drive()");a.speedcheck&&(a.log.speed.fire=b.timeStamp);a.speedcheck&&(a.log.speed.time=[]);a.speedcheck&&(a.log.speed.name=[]);a.speedcheck&&a.log.speed.name.push("fire");a.speedcheck&&a.log.speed.time.push(a.speed.now()-a.log.speed.fire);if(!1===k(a.callbacks.before,g,[b,a.parameter]))return g;if(f)jQuery.when?jQuery.when(A(a.wait)).done(function(){s(f)}):s(f);else{var l,
h,d,q,p,B,C=[],y={xhr:function(){d=k(a.callbacks.ajax.xhr,g,[b,a.parameter]);d=d instanceof Object&&d instanceof c.XMLHttpRequest?d:d||jQuery.ajaxSettings.xhr();d instanceof Object&&(d instanceof c.XMLHttpRequest&&"onprogress"in d)&&d.addEventListener("progress",function(a){B=a.loaded},!1);return d},dataFilter:function(d,e){l=d;h=e;return k(a.callbacks.ajax.dataFilter,g,[b,a.parameter,l,h])||l},complete:function(e,c){d=e;q=c;k(a.callbacks.ajax.complete,g,[b,a.parameter,d,q])}},x;for(x in y)x in a.callbacks.ajax||
...
d])},success:function(e,c,n){l=e;h=c;d=n;k(a.callbacks.ajax.success,g,[b,a.parameter,l,h,d])},error:function(n,f,h){d=n;q=f;p=h;k(a.callbacks.ajax.error,g,[b,a.parameter,d,q,p]);j&&j.test("++",1,[e,c.location.href],"ajax error");j&&j.end();if(a.fallback)return"function"===typeof a.fallback?a.fallback(b):G(b)}}))).done(function(){s()}).fail().always():jQuery.ajax(jQuery.extend(!0,{},a.ajax,y,{url:e,beforeSend:function(e){d=e;d.setRequestHeader(a.nss.requestHeader,"true");d.setRequestHeader(a.nss.requestHeader+
"-Area",a.area);d.setRequestHeader(a.nss.requestHeader+"-CSS",a.load.css);d.setRequestHeader(a.nss.requestHeader+"-Script",a.load.script);k(a.callbacks.ajax.beforeSend,g,[b,a.parameter,d])},success:function(e,c,n){l=e;h=c;d=n;k(a.callbacks.ajax.success,g,[b,a.parameter,l,h,d]);s()},error:function(n,f,h){d=n;q=f;p=h;k(a.callbacks.ajax.error,g,[b,a.parameter,d,q,p]);j&&j.test("++",1,[e,c.location.href],"ajax error");j&&j.end();if(a.fallback)return"function"===typeof a.fallback?a.fallback(b):G(b)}}));
