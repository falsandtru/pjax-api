...
g.fallback(h);var p,w,n,s;try{c.xhr&&4>c.xhr.readyState&&c.xhr.abort();c.xhr=null;if(!k&&!~(q.getResponseHeader("Content-Type")||"").toLowerCase().search(c.contentType))throw Error("throw: content-type mismatch");k&&!1!==g.fire(r.cache.load.before,null,[h,c.parameter,k],c.callbacks.async)&&(q=k.XMLHttpRequest||q,u=q.responseText,v=k.textStatus||v,n=k.css,s=k.script,g.fire(r.cache.load.after,null,[h,c.parameter,k],c.callbacks.async));k&&k.XMLHttpRequest||!c.cache.click&&!c.cache.submit&&!c.cache.popstate||
"submit"===h.type.toLowerCase()&&!c.cache[h.target.method.toLowerCase()]||!1===g.fire(r.cache.save.before,null,[h,c.parameter,k],c.callbacks.async)||(a[g.name].setCache(l,k&&k.data||null,v,q),k=a[g.name].getCache(l),g.fire(r.cache.save.after,null,[h,c.parameter,k],c.callbacks.async));var t,y,z,E;t=g.createHTMLDocument(q.responseText);var D=g.chooseAreas(t,h,c.areaback,c.parameter,c.destination.href,c.location.href);c.areas=a.map(c.areas,function(b){return~a.inArray(b,D)?b:null});c.area=c.areas[0];
if(!c.area||a(c.area).length!==a(c.area,t).length)throw Error("throw: area length mismatch");p=a("title",t).text();z=c.area.match(/(?:[^,\(\[]+|\(.*?\)|\[.*?\])+/g);if(k&&k.data){y=g.createHTMLDocument(k.data);for(var H=0,I,A,K;I=z[H++];){A=a(I,t);K=a(I,y);for(var F=0;x=K[F];F++)A.eq(F).html(a(x).contents())}}m&&c.log.speed.time.push(c.speed.now()-c.log.speed.fire);m&&c.log.speed.name.push("parse("+c.log.speed.time[c.log.speed.time.length-1]+")");a("noscript",t).children().parent().each(function(){this.children.length&&
